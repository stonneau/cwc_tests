...
def gen_sequence_data_from_state(fullBody, stateid, configs, m = 55.88363633, mu = 0.5):    
    ...
    states = []
    states += [make_state(fullBody, Ps[0], Ns[0], fullBody.getConfigAtState(stateid)  )]
    states += [make_state(fullBody, Ps[-1], Ns[-1], fullBody.getConfigAtState(stateid+1))]    
...
def gen_sequence_data_from_state_objects(s1,s2,sInt,mu = 0.5):
    ...
    print "KO" 
    K0 = asarray(compute_CWC(Ps1[0], Ns1[0], mass=m, mu = mu, simplify_cones = False))
...
def generate_problem(data, test_quasi_static = False, m = 55.88363633, mu = 0.5):
    ...
    bounds_c = [[min(c0[0], c1[0])-0.03, max(c0[0], c1[0])+0.03]]
    bounds_c += [[min(c0[1], c1[1])-0.06, max(c0[1], c1[1])+0.00]]
    bounds_c += [[min(c0[2], c1[2])-0.03, max(c0[2], c1[2])+0.03]]
...
def project_com_colfree(fullBody, stateid, com):
    try:
        print "before project", stateid, com
...
def add_z_constraints(Kin, c_sample_bounds):   
    A = Kin[0]
    b = Kin[1]
    print "z constraints ", c_sample_bounds[4]
    #~ A =zeros([A.shape[0],A.shape[1]]) # TODO
    #~ b =zeros([b.shape[0]])
...
def add_y_constraints(Kin, c_sample_bounds):   
    A = Kin[0]
    b = Kin[1]
    A_Kin = zeros([A.shape[0]+2,A.shape[1]])
    b_Kin = zeros([b.shape[0]+2])
    ...
    b_Kin[:A.shape[0]]   = b[:]
    A_Kin[A.shape[0],1] = 1
    b_Kin[A.shape[0]] = c_sample_bounds[3]
    A_Kin[A.shape[0]+1,1] = -1
...
def solve_quasi_static(data, c_bounds, c_sample_bounds = None, use_rand = False, m = 55.88363633, mu = 0.5, fullBody = None):
    ...
    if(c_sample_bounds == None):
        ...
        c_sample_bounds += [min(c0[1], c1[1])-0.08, max(c0[1], c1[1])+0.08]
        c_sample_bounds += [min(c0[2], c1[2]), max(c0[2], c1[2])+0.2]
    if use_rand:
